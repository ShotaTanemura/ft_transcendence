...
class UserRegisterTest(TestCase):
    def test_register_normal(self):
        ...
        response = self.client.post(
            reverse("pong:register"), data=data, content_type="application/json"
        )
    ...
    def test_register_no_email(self):
        data = {"name": "ユーザー名", "password": "p4s$W0rd"}
        response = self.client.post(
            reverse("pong:register"), data=data, content_type="application/json"
        )
        self.assertEqual(response.json()["message"], "Invalid parameters")
...
class UserTokenTest(TestCase):
    def test_token_normal(self):
        ...
        data = {"email": "example@email.com", "password": "p4s$W0rd"}
        response = self.client.post(
            reverse("pong:token"), data=data, content_type="application/json"
        )
        ...
        try:
            decoded_token = jwt.decode(
                token, settings.JWT_AUTH["JWT_PUBLIC_KEY"], algorithms=["RS256"]
            )
        ...
        try:
            decoded_refresh_token = jwt.decode(
                refresh_token, settings.JWT_AUTH["JWT_PUBLIC_KEY"], algorithms=["RS256"]
            )
        except jwt.ExpiredSignatureError:
...
class UserRefreshTokenTest(TestCase):
    ...
    def test_refresh_token_normal(self):
        ...
        response = self.client.post(
            reverse("pong:refresh"), content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
...
class UserVerifyTokenTest(TestCase):
    ...
    def test_verify_token_normal(self):
        response = self.client.post(
            reverse("pong:verify"), content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["uuid"], str(self.user.uuid))
