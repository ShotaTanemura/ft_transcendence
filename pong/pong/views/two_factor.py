...
def register(request):
    ...
    if (tfa.is_active == True): tfa.secret = generate_base32_encoded_raondom_string()
    else: tfa.is_active = True
    tfa.save()

    # Generate
    uri = generateOtpUri(secret=tfa.secret, account_name=settings.DJANGO_2FA_ACCOUNT_NAME, issuer=settings.DJANGO_2FA_ISSUER)

    return JsonResponse({"uri": uri}, status=200)

def generateOtpUri(secret, account_name, issuer, digits=6, period=30, algorithm='SHA256'):
