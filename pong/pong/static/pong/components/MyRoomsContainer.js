...
export class MyRoomsContainer extends Component {
    constructor(router, params, state, onRoomSelect) {
        super(router, params, state, '.myrooms-container');
...
updateMessages(myrooms) {
    ...
    myroomsContainer.querySelectorAll('.myroom').forEach(roomElement => {
        console.log('Adding event listener to: ', roomElement);
        roomElement.addEventListener('click', () => {
            myroomsContainer.querySelectorAll('.myroom').forEach(el => el.classList.remove('selected'));
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        form.addEventListener('submit', async (event) => {
            ...
            if (response.ok) {
                ...
                modal.style.display = 'none';
                await this.initMessages(); // 新しいチャットルームを表示するためにメッセージを再取得
            } else {
