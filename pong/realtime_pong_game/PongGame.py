...
class Paddle:
    ...
    def move(self):
        if self.position - self.size / 2 + self.velocity < 0 or FIELD_HEIGHT < self.position + self.size / 2 + self.velocity:
            return 
...
class PongGame:
    ...
    def handle_collision(self):
        ...
        if self.ball.x_position <= GRID and self.player1_paddle.position - PADDLE_SIZE / 2 <= self.ball.y_position and self.ball.y_position <= self.player1_paddle.position + PADDLE_SIZE / 2:
            self.ball.x_velocity = -1 * self.ball.x_velocity
        elif FIELD_WIDTH - GRID <= self.ball.x_position and self.player2_paddle.position - PADDLE_SIZE / 2 <= self.ball.y_position and self.ball.y_position <= self.player2_paddle.position + PADDLE_SIZE / 2:
            self.ball.x_velocity = -1 * self.ball.x_velocity
